...
from data_utils import load_depth, load_composed_depth, get_bbox, fill_missing, get_bbox_from_mask
...
class TrainingDataset(Dataset):
    def __init__(self, config, data_dir, data_type='real', num_img_per_epoch=-1, use_fill_miss=True, use_composed_img=True, per_obj=''):
        ...
        if data_type == "syn":
            ...
            self.text_name = {'1': 'synthetic bottle',
                '2': 'synthetic bowl',
                '3': 'synthetic camera',
                '4': 'synthetic can',
                '5': 'synthetic laptop',
            ...
            self.text_name = {'1': 'bottle', '2': 'bowl', '3': 'camera', '4': 'can', '5': 'laptop', '6': 'mug'}
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == 'syn' and self.use_composed_img:
            ...
            depth = load_composed_depth(img_path) #flase
        else:
            # depth = load_depth(img_path)
            depth = np.load(img_path + '_dinov2_depth.npy')
        ...
        if self.use_fill_miss:
            ...
            depth = fill_missing(depth, 1, 1) #Ture
        ...
        # if cat_id == 0:
        # if self.data_type == 'syn' and cat_id == 0:
        #     continue
     
        one_hot = one_hot_encode(cat_id+1)
        prior = np.load('/mnt/hdd3/yh/ist/assets/mean_points_emb.npy')
        prior = prior[cat_id]
        ...
        if self.data_type == 'syn' or self.data_type == 'real_withLabel':
            ...
            ret_dict['one_hot'] = torch.FloatTensor(one_hot)
            ...
            r_vector = cv2.Rodrigues(rotation)[0]
            r = cv2.Rodrigues(r_vector)[0]





            text = 'a object of a {class_name}'\
                 .format(class_name = self.text_name[str(cat_id + 1)])
            text = clip.tokenize(text)

            ret_dict['text'] = text

            text_points = 'a point cloud of a {class_name}'\
                 .format(class_name = self.text_name[str(cat_id + 1)])
            text_points = clip.tokenize(text_points)
            ret_dict['text_points'] = text_points   
  
            text_img = 'a image of a {class_name}'\
                 .format(class_name = self.text_name[str(cat_id + 1)])
            text_img = clip.tokenize(text_img)
            ret_dict['text_img'] = text_img  
...
class TestDataset():
    def __init__(self, config, data_dir):
        ...
        # result_pkl_list = glob.glob(os.path.join(self.data_dir, 'data', 'segmentation_results_refine_for_mug', 'REAL275', 'results_*.pkl'))
    ...
    def __getitem__(self, index):
        ...
        # mrcnn_path = os.path.join('data/segmentation_results/test_trainedwoMask', 'results_test_{}_{}.pkl'.format(
        ...
        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! segmented by CAMERA or CAMERA+Real
        ...
        # pts
        cam_fx, cam_fy, cam_cx, cam_cy = self.intrinsics
        depth = np.load(image_path + '_dinov2_large_depth.npy')
        depth = depth.squeeze(0).squeeze(0)
        # depth = depth

        # depth = fill_missing(depth, 1, 1)
        ...
        for j in range(num_instance):
            ...
            if len(choose)>16:
                ...
                instance_rgb = cv2.resize(instance_rgb, (224,224), interpolation=cv2.INTER_LINEAR)
                ...
                all_one_hot.append(torch.FloatTensor(one_hot))
                all_text.append(text)
                all_text_points.append(text_points)
                all_text_img.append(text_img)
        ...
        ret_dict['one_hot'] = torch.stack(all_one_hot)
        ret_dict['rgb'] = torch.stack(all_rgb)
        ret_dict['text'] = torch.stack(all_text)
        ret_dict['text_points'] = torch.stack(all_text_points)
        ret_dict['text_img'] = torch.stack(all_text_img)
        ...
        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! segmanted by ImageNet Mask-Rcnn
        # num_instance = len(pred_data['pred_class_ids'])
        ...
        # # with open(img_path + '_label.pkl', 'rb') as f:
        # #     gts = cPickle.load(f)
        # # ret_dict['gt_class_ids'] = gts['class_ids']
        # # ret_dict['gt_bboxes'] = gts['bboxes']
        # # ret_dict['gt_RTs'] = gts['poses']
        # # ret_dict['gt_scales'] = gts['size']
        # # ret_dict['gt_handle_visibility'] = gts['handle_visibility']
...
class TestCAMERADataset():
    def __init__(self, config, data_dir):
        ...
        # result_pkl_list = glob.glob(os.path.join(self.data_dir, 'data', 'mrcnn_results', dataset, 'results_*.pkl'))
        result_pkl_list = glob.glob(os.path.join(self.data_dir, 'data', 'segmentation_results_refine_for_mug', 'CAMERA25', 'results_*.pkl'))
        self.result_pkl_list = sorted(result_pkl_list)
        n_image = len(result_pkl_list)
        print('no. of test images: {}\n'.format(n_image))
        ...
        self.transform = transforms.Compose([transforms.ToTensor(),
                                             transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                  std=[0.229, 0.224, 0.225])])
        self.class_name_map = {1: 'synthetic bottle',
                               2: 'synthetic bowl',
                               3: 'synthetic camera',
                               4: 'synthetic can',
                               5: 'synthetic laptop',
    ...
    def __getitem__(self, index):
        ...
        image_path = image_path.replace('/data/camera/', '/data/CAMERA/') #always
        ...
        # pts
        cam_fx, cam_fy, cam_cx, cam_cy = self.intrinsics
        if os.path.exists(image_path + '_dinov2_large_depth.npy'):
            depth = np.load(image_path + '_dinov2_large_depth.npy')
        else:
            depth = np.load(image_path + '_dinov2_depth.npy')
        depth = depth.squeeze(0).squeeze(0)
        if self.dataset == 'real_test':
            depth = fill_missing(depth, 1, 1)
