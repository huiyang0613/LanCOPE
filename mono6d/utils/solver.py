...
class Solver(gorilla.solver.BaseSolver):
   ...
   # real
    ...
    #     for real_data in self.dataloaders["real"]:
    #         data_time = time.time()-end
    ...
    #         self.optimizer.zero_grad()
    #         loss, dict_info_step = self.step(real_data, mode)
    #         forward_time = time.time()-end-data_time
    ...
    #             self.log_buffer.average(self.per_write)
    #             prefix = '[{}/{}][{}/{}][{}] Train - '.format(
    #                 self.epoch, self.cfg.max_epoch, i, len(self.dataloaders["real"]), self.iter)
    #             write_info = self.get_logger_info(
    #                 prefix, dict_info=self.log_buffer._output)
    #             self.logger.info(write_info)
    #             self.write_summary(self.log_buffer._output, mode)
    ...
    def step(self, syn_data, real_data, mode):
        ...
        data = {
            ...
            'one_hot': torch.cat([syn_data['one_hot'], real_data['one_hot']], dim=0),
            'prior': torch.cat([syn_data['prior'], real_data['prior']], dim=0),
            'text': torch.cat([syn_data['text'], real_data['text']], dim=0),
            'text_points': torch.cat([syn_data['text_points'], real_data['text_points']], dim=0),
            'text_img': torch.cat([syn_data['text_img'], real_data['text_img']], dim=0),
...
# ########################    nce  loss  start  #################################################            
#         loss_syn_tince = torch.mean(syn_data['text2image_loss'])
#         loss_syn_tpnce = torch.mean(syn_data['text2point_loss'])

#         loss_real_tince = torch.mean(real_data['text2image_loss'])
#         loss_real_tpnce = torch.mean(real_data['text2point_loss'])

#         loss_syn_nce = loss_syn_tince + loss_syn_tpnce
#         loss_real_nce = loss_real_tince + loss_real_tpnce

#         loss_nce_all =  (loss_syn_nce * b1 + loss_real_nce * b2) / (b1+b2)
########################    nce  loss    end #################################################            
...
########################    nce  loss  #################################################            
            # "loss_syn_nce": float(loss_syn_nce.item()),
            # "loss_real_nce": float(loss_real_nce.item())
        }
    ...
    #         # 'rgb1': syn_data['rgb1'],
    #         'pts': syn_data['pts'],
    #         'text_points': syn_data['text_points'],
    #         'text_img': syn_data['text_img'],
    #         'text': syn_data['text'],
    ...
    # # real
    # def step(self, real_data, mode):
    #     torch.cuda.synchronize()
    #     b1 = real_data['rgb'].size(0)
    ...
    #         # 'rgb1': syn_data['rgb1'],
    #         'pts': real_data['pts'],
    #         'text_points': real_data['text_points'],
    #         'text_img': real_data['text_img'],
    #         'text': real_data['text'],



    #         'choose': real_data['choose'],
    #         'category_label': real_data['category_label'],
    #         'model': real_data['model'],
    #         'sym_info': real_data['sym_info'],
    #         'gt_R': real_data['rotation_label'],
    #         'gt_t': real_data['translation_label'],
    #         'gt_s': real_data['size_label'],
    #         'qo': real_data['qo'],
    #     }
    #     end_points = self.model(data)

    #     for key in end_points:
    #         real_data[key] = end_points[key][0:b1]

    #     loss_real = self.loss['real'](real_data) # actually is supervised !!!!!!
        ...
        for i, data in enumerate(dataloder):
            ...
            if 'rgb' in  data.keys():
                ...
                inputs = {
                    'rgb': data['rgb'][0].cuda(),
                    'pts': data['pts'][0].cuda(),
                    # 'one_hot': data['one_hot'][0].cuda(),
                    'text': data['text'][0].cuda(),
                    'text_points': data['text_points'][0].cuda(),
                    'text_img': data['text_img'][0].cuda(),
...
def draw_box_to_image(data, result,  data_name, img_id):
    ...
    out_dir = "/mnt/HDD2/lj/DIF6D/log1/vis_real/"
